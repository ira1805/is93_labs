...
// Функція отримання даних (точок) трикутника
vector<vector<int>> Triangle::Get() {
	return points;
};

// Перевантажена функція отримання даних трикутника, повертає одне значення матриці точок
...
// Приватна функція, використовується для розрахунку периметра
